...
function classify_Kfold(sample_info_path,bed_col,pos_neg_col,fold,bed_folder,X,Y,readlen,buchang,gp,fdr,mat_path)

[~,~,info] = xlsread(sample_info_path);
sample_name = info(:,bed_col);
...
cfdnapath = mat_path;
parfor i = 1:size(sample_name,1)
    
    bed_name =cell2mat(sample_name(i));
